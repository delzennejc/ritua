...
function detachInactiveReferencesView(fields: Fields, ids: string[]) {
  ...
  for (const session of (next.events ?? []) as Data[]) {
    ...
    session.taskIds = (session.taskIds as string[]).filter((id, index) => {
      ...
      removed.push({ field: 'events', projectId: String(session.id), index, member: { id } })
      return false
    })
  }
  return { fields: next, removed }
}
export function detachInactiveReferences(document: WorkspaceDocument, ids: string[]) {
  return collectionCommand(document, (fields) => detachInactiveReferencesView(fields, ids))
}
function restoreInactiveReferencesView(fields: Fields, removed: RemovedReference[]): Fields {
  const next = fields
  for (const entry of removed) {
    ...
    if (entry.field === 'events') {
      ...
      if (!ids.includes(String(entry.member.id)))
        ids.splice(Math.min(entry.index, ids.length), 0, String(entry.member.id))
      continue
    }
    const members = (project.tasks ??= []) as Data[]
    if (!members.some((item) => (item.taskId ?? item.id) === (entry.member.taskId ?? entry.member.id)))
      members.splice(Math.min(entry.index, members.length), 0, copyRecord(entry.member))
  }
  return next
}
export function restoreInactiveReferences(document: WorkspaceDocument, removed: RemovedReference[]) {
  return collectionCommand(document, (fields) => restoreInactiveReferencesView(fields, removed))
}
function restoreAreaView(fields: Fields, id: string): Fields {
  const next = fields,
    archived = (next.archivedAreas ?? []) as Data[]
  const entry = archived.find((item) => (item.area as Data).id === id)
  if (!entry) return next
  const area = entry.area as Data,
    areas = next.areas as Data[]
  if (
    areas.some(
      (item) =>
        item.id === id ||
        String(item.label).localeCompare(String(area.label), undefined, { sensitivity: 'accent' }) === 0,
    )
  )
    throw new Error('An active Area already uses this name. Rename it before restoring this Area.')
  areas.splice(Math.min(Number(entry.position), areas.length), 0, area)
  next.archivedAreas = archived.filter((item) => item !== entry)
  return next
}
export function restoreArea(document: WorkspaceDocument, id: string) {
  return collectionCommand(document, (fields) => restoreAreaView(fields, id))
}
function restoreProjectView(fields: Fields, id: string): Fields {
  let next = fields
  ...
  const hiddenArea = ((next.archivedAreas ?? []) as Data[]).find(
    (item) => (item.area as Data).label === label,
  )
  if (hiddenArea) next = restoreAreaView(next, String((hiddenArea.area as Data).id))
  const active = next.weeklyObjectives as Data[]
  if (active.some((item) => item.id === id)) throw new Error('This Project is already active.')
  const tasks = [
    ...(next.tasks as Data[]),
    ...Object.values(next.datedTasksByDate as Record<string, Data[]>).flat(),
    ...((next.backlogGroups ?? []) as Data[]).flatMap((group) => group.items as Data[]),
  ]
  const restored = copyRecord(project)
  ...
  delete restored.archivedSeriesIds
  delete restored.archiveWeeklyOrderIndex
  delete restored.archivePosition
  delete restored.archivedAt
  active.splice(Math.min(position, active.length), 0, restored)
  next.archivedObjectives = archived.filter((item) => item.id !== id)
  return next
}
export function restoreProject(document: WorkspaceDocument, id: string) {
  return collectionCommand(document, (fields) => restoreProjectView(fields, id))
}
