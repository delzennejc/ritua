...
const pluralUnit = (unit: string, amount: number) => (amount === 1 ? unit : `${unit}s`)
...
const normalizedEnd = (end: Partial<Recurrence['end']> = {}): Recurrence['end'] => ({
  type: end.type === 'on' || end.type === 'after' ? end.type : 'never',
  date: end.date || '',
  count: Math.max(1, Number(end.count) || 10),
})

export const noRecurrence = (): Recurrence => ({
...
export const recurrenceForPreset = (
  preset: Recurrence['preset'],
  dateKey: string,
  current = noRecurrence(),
): Recurrence => {
...
export const recurrenceLabel = (recurrence: Recurrence, dateKey: string) => {
...
export const customRecurrenceForChange = (
  recurrence: Recurrence,
  change: Partial<Omit<Recurrence, 'end'>> & { end?: Partial<Recurrence['end']> },
  dateKey: string,
): Recurrence => {
  const next: Recurrence = {
    ...recurrenceForPreset(RECURRENCE_PRESETS.CUSTOM, dateKey, recurrence),
    ...change,
    end: normalizedEnd({ ...recurrence.end, ...change.end }),
    preset: RECURRENCE_PRESETS.CUSTOM,
  }
...
const monthlyCandidate = (start: Date, monthOffset: number, monthMode: Recurrence['monthMode']) => {
...
export const recurrenceDateKeys = (
  ...
  const horizon =
    end.type === 'on' && end.date && end.date < visibleHorizonDateKey
      ? dateFromKey(end.date)
      : visibleHorizon < start
        ? start
        : visibleHorizon
  const targetCount = end.type === 'after' ? end.count : Number.POSITIVE_INFINITY
  const results = [startDateKey]
  const interval = Math.max(1, Number(recurrence.interval) || 1)
  const canAdd = (candidate: Date | null): candidate is Date =>
    candidate !== null && candidate > start && candidate <= horizon && results.length < targetCount
  const addCandidate = (candidate: Date | null) => {
  ...
  if (recurrence.frequency === 'day') {
    ...
    for (let cycle = 1; cycle < 100000 && results.length < targetCount; cycle += 1) {
      ...
      if (!addCandidate(candidate) && candidate && candidate > horizon) break
    }
    ...
    for (let cycle = 1; cycle < 100000 && results.length < targetCount; cycle += 1) {
      ...
      const validDate =
        candidate.getFullYear() === year && candidate.getMonth() === start.getMonth() ? candidate : null
      if (!addCandidate(validDate) && validDate && validDate > horizon) break
    }
  }
