...
import type { IncompletePosition } from './models'
import type { Task } from './models'
export const completedTasksLast = <T extends Task>(tasks: T[]) => [
...
export const nextScheduledOccurrences = <T extends Task>(scheduledTasks: T[]) => {
...
export const setTaskCompletionInObjectiveMirrors = (
  ...
  objectives.map((objective) => ({
    ...objective,
    tasks: (() => {
      const sourceTask = objective.tasks?.find((task) => task.taskId === taskId || task.id === taskId)
      if (!sourceTask || sourceTask.complete === complete) return objective.tasks

      return toggleTaskInTasks(objective.tasks!, sourceTask.id || sourceTask.taskId!)
    })(),
  }))

const minutesFromTimeLabel = (time: unknown) => {
...
export const orderTasksByTime = <T extends Task>(tasks: T[]) =>
  tasks
    .map((task, index) => ({
      task,
      index,
    ...
    .sort((first, second) => {
      ...
      return (first.start ?? 0) - (second.start ?? 0) || first.index - second.index
    })
    .map(({ task }) => task)

export const setTaskTimeInTasks = (tasks: Task[], taskId: string, time: string | null) => {
  if (!tasks.some((task) => task.id === taskId)) return tasks

  return orderTasksByTime(tasks.map((task) => (task.id === taskId ? { ...task, time } : task)))
}

export const insertBeforeCompletedTasks = <T extends { complete?: boolean }>(tasks: T[], task: T) => {
  const firstCompletedIndex = tasks.findIndex((item) => item.complete)
  const insertionIndex = firstCompletedIndex === -1 ? tasks.length : firstCompletedIndex

  return [...tasks.slice(0, insertionIndex), task, ...tasks.slice(insertionIndex)]
}

const incompletePositionFor = (tasks: Task[], taskId: string) => {
...
const restoreIncompleteTask = (tasks: Task[], task: Task, position?: IncompletePosition | null) => {
  if (!position) return insertBeforeCompletedTasks(tasks, task)

  const nextTaskIndex = position.nextTaskId
    ? tasks.findIndex((item) => !item.complete && item.id === position.nextTaskId)
    : -1
  if (nextTaskIndex !== -1) {
    return [...tasks.slice(0, nextTaskIndex), task, ...tasks.slice(nextTaskIndex)]
  }

  const previousTaskIndex = position.previousTaskId
    ? tasks.findIndex((item) => !item.complete && item.id === position.previousTaskId)
    : -1
  if (previousTaskIndex !== -1) {
    return [...tasks.slice(0, previousTaskIndex + 1), task, ...tasks.slice(previousTaskIndex + 1)]
  }
...
export const toggleTaskInTasks = (tasks: Task[], taskId: string, completedAtMinute?: number) => {
  ...
  const resolvedCompletionMinute = Number.isFinite(completedAtMinute)
    ...
    : currentMinuteOfDay()
  const incompletePosition = complete ? incompletePositionFor(tasks, taskId) : sourceTask.incompletePosition
    ...
    ...(complete && sourceTask.subtasks?.length
      ? {
          subtasks: sourceTask.subtasks.map((subtask) => ({
            ...subtask,
            complete: true,
            completedAtMinute: subtask.complete ? subtask.completedAtMinute : resolvedCompletionMinute,
          })),
        }
      : {}),
  }
...
export const completeUndatedTaskInTasks = (tasks: Task[], sourceTask: Task, completedAtMinute?: number) => {
  const task = {
    ...sourceTask,
    complete: false,
    minutes: Number.isFinite(sourceTask.minutes) && sourceTask.minutes! > 0 ? sourceTask.minutes : 15,
  }
