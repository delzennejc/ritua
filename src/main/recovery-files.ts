...
import { durableFields, validateJson, validateDocument, validateSnapshot, type WorkspaceDocument, type Entity } from '../domain/workspace'
...
export interface StoredAttachment { id: string; name: string; size: number; sha256: string; content: Buffer }
export function validateRecovery(value: unknown): asserts value is RecoveryDraft {
  ...
  if (!draft?.base || !draft.local || typeof draft.savedAt !== 'string' || JSON.stringify(value).length > 32_000_000) throw new Error('Invalid recovery draft')
  for (const doc of [draft.base, draft.local]) {
    ...
    for (const key of Object.keys(doc.fields)) if (![...durableFields, 'dateKeys'].includes(key as typeof durableFields[number])) throw new Error('Invalid recovery field')
  }
}
export function readBackup(filename: string) {
  ...
  try {
    ...
    for (const table of ['workspace_state', 'workspace_entities']) {
      if (!source.prepare("SELECT 1 FROM sqlite_master WHERE name=? AND type='table' AND sql NOT LIKE '%VIRTUAL%'").get(table)) throw new Error('This is not a Ritua workspace backup')
    }
    const state = source.prepare('SELECT revision, fields FROM workspace_state WHERE id=1').get() as { revision: number; fields: string } | undefined
    if (!state || state.fields.length > 12_000_000) throw new Error('Invalid backup state')
    const rows = source.prepare('SELECT kind, entity_id, data FROM workspace_entities LIMIT 100001').all() as { kind: Entity['kind']; entity_id: string; data: string }[]
    const document: WorkspaceDocument = { revision: state.revision, fields: JSON.parse(state.fields), entities: rows.map(row => ({ kind: row.kind, id: row.entity_id, data: JSON.parse(row.data) })) }
    ...
    if (source.prepare("SELECT 1 FROM sqlite_master WHERE name='attachments' AND type='table'").get()) {
      const total = source.prepare('SELECT COALESCE(SUM(size),0) AS size FROM attachments').get() as { size: number }
      if (total.size > 512 * 1024 * 1024) throw new Error('Backup attachments exceed the restore limit')
      attachments = source.prepare('SELECT id,name,size,sha256,content FROM attachments').all() as StoredAttachment[]
      for (const file of attachments) {
        if (!/^[a-zA-Z0-9-]{1,100}$/.test(file.id) || typeof file.name !== 'string' || file.name.length > 255 || !Buffer.isBuffer(file.content) || file.size !== file.content.length || file.size > MAX_ATTACHMENT_BYTES || createHash('sha256').update(file.content).digest('hex') !== file.sha256) throw new Error('Backup contains an invalid attachment')
      }
    }
...
export function recoveryFiles(directory: string, database: ReturnType<typeof openDatabase>) {
  ...
  const writeRecovery = (draft: RecoveryDraft | null) => {
    if (draft) validateRecovery(draft)
    const next = journalQueue.catch(() => {}).then(async () => {
      if (!draft) { await unlink(journal).catch(error => { if (error.code !== 'ENOENT') throw error }); return }
  ...
  const readRecovery = async () => {
    ...
    try { const value: unknown = JSON.parse(await readFile(journal, 'utf8')); validateRecovery(value); return value }
    catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return null; const preserved = `${journal}.preserved-${Date.now()}`; await rename(journal, preserved); throw new Error(`The unreadable recovery copy was preserved at ${preserved}. Your saved workspace is intact.`) }
  }
  const listBackups = async (): Promise<BackupInfo[]> => {
    await mkdir(folder, { recursive: true, mode: 0o700 })
    const entries = await readdir(folder)
    return (await Promise.all(entries.filter(name => /^ritua-[0-9T-]+-[a-z-]+\.sqlite$/.test(name)).map(async id => {
  ...
  const backupPath = (id: string) => {
    if (!/^ritua-[0-9T-]+-[a-z-]+\.sqlite$/.test(id) || basename(id) !== id) throw new Error('Invalid backup selection')
    return join(folder, id)
  }
  const createBackup = async (reason = 'manual') => {
    let result: BackupInfo | undefined
    const next = backupQueue.catch(() => {}).then(async () => {
      await mkdir(folder, { recursive: true, mode: 0o700 })
