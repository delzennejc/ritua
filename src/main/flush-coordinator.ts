...
export class FlushCoordinator {
  ...
  constructor(private send: (id: string) => void, private timeoutMs = 8000) {}
  request(): Promise<boolean> {
    ...
    this.pending = { id, promise, finish: success => { clearTimeout(timer); this.pending = undefined; complete(success) } }
  ...
  rendererGone() { if (this.pending) this.pending.finish(false) }
}
