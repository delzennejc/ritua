...
export function openDatabase(filename: string) {
  ...
  try {
    ...
    db.insert(appMetadata)
      .values({ key: 'initializedAt', value: new Date().toISOString() })
      .onConflictDoNothing()
      .run()
    if (!db.select().from(workspaceState).get()) {
      ...
      db.transaction((tx) => {
        if (initial.entities.length)
          tx.insert(workspaceEntities)
            .values(
    ...
    const loadWorkspace = (): WorkspaceDocument => {
      ...
      return {
        ...
        entities: db
          ...
          .all()
          .map((e) => ({ kind: e.kind as Entity['kind'], id: e.entityId, data: e.data })),
      }
    }
    {
        ...
        db.transaction((tx) => {
          ...
          if (next.entities.length)
            tx.insert(workspaceEntities)
              ...
              .run()
          tx.update(workspaceState)
            .set({ revision: before.revision + 1, fields: next.fields })
            .where(eq(workspaceState.id, 1))
            .run()
        })
    }
    const reconcileAttachments = (protectedIds = new Set<string>(), now = Date.now()) => {
      const live = attachmentIds(loadWorkspace())
      for (const id of protectedIds) live.add(id)
      const rows = sqlite.prepare('SELECT id,unreferenced_since FROM attachments').all() as {
        id: string
        unreferenced_since: number
      }[]
      for (const row of rows) {
        if (live.has(row.id))
          sqlite.prepare('UPDATE attachments SET unreferenced_since=0 WHERE id=?').run(row.id)
        else if (!row.unreferenced_since)
          sqlite.prepare('UPDATE attachments SET unreferenced_since=? WHERE id=?').run(now, row.id)
      }
      return live
    }
    ...
    return {
      ...
      attachmentStorage() {
        const row = sqlite
          .prepare('SELECT COALESCE(SUM(size),0) AS used, COUNT(*) AS files FROM attachments')
          .get() as { used: number; files: number }
        return { ...row, limit: 512 * 1024 * 1024 }
      },
      ...
      collectAttachments(protectedIds = new Set<string>(), now = Date.now()) {
        ...
        sqlite
          .prepare('DELETE FROM attachments WHERE unreferenced_since>0 AND unreferenced_since<?')
      ...
      commitWorkspace(raw: unknown): { revision: number } {
        ...
        return db.transaction((tx) => {
          const receipt = tx
            .select()
            .from(workspaceReceipts)
            .where(eq(workspaceReceipts.id, command.requestId))
            .get()
          if (receipt) {
            if (receipt.digest !== digest)
              throw new WorkspaceValidationError('Request ID was reused for a different change')
            return { revision: receipt.revision }
          }
          const result = applyChanges(loadWorkspace(), command)
          for (const e of command.remove)
            tx.delete(workspaceEntities)
              .where(eq(workspaceEntities.key, `${e.kind}:${e.id}`))
              .run()
          for (const e of command.put) {
            const row = { key: `${e.kind}:${e.id}`, kind: e.kind, entityId: e.id, data: e.data }
            tx.insert(workspaceEntities)
              .values(row)
              .onConflictDoUpdate({ target: workspaceEntities.key, set: row })
              .run()
          }
          tx.update(workspaceState)
            .set({ revision: result.revision, fields: result.fields })
            .where(eq(workspaceState.id, 1))
            .run()
          reconcileAttachments()
          tx.insert(workspaceReceipts)
            .values({ id: command.requestId, revision: result.revision, digest })
            .run()
          tx.delete(workspaceReceipts)
            .where(lt(workspaceReceipts.revision, result.revision - 256))
            .run()
      ...
      addAttachment(value: typeof attachments.$inferInsert) {
        const total = sqlite.prepare('SELECT COALESCE(SUM(size),0) AS size FROM attachments').get() as {
          size: number
        }
        if (total.size + value.size > 512 * 1024 * 1024)
          throw new WorkspaceValidationError('The workspace attachment limit is 512 MB.')
        db.insert(attachments)
          .values({ ...value, unreferencedSince: Date.now() })
          .run()
      },
        ...
        db.transaction((tx) => {
          ...
          if (document.entities.length)
            tx.insert(workspaceEntities)
              ...
              .run()
          tx.update(workspaceState)
            .set({ revision, fields: document.fields })
            .where(eq(workspaceState.id, 1))
            .run()
