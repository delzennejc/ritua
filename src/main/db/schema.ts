...
export const appMetadata = sqliteTable('app_metadata', { key: text('key').primaryKey(), value: text('value').notNull() })
export const workspaceEntities = sqliteTable('workspace_entities', {
  key: text('key').primaryKey(), kind: text('kind').notNull(), entityId: text('entity_id').notNull(),
...
export const workspaceReceipts = sqliteTable('workspace_receipts', {
  id: text('id').primaryKey(), revision: integer('revision').notNull(), digest: text('digest').notNull(),
})
