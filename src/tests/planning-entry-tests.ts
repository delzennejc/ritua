...
import { completeWeeklyPlanning, openPendingPlanning } from '../domain/planning-entry'
...
export function testPlanningEntry() {
  ...
  assert.equal(openPendingPlanning({ ...fields, 'daily.completedDate': monday }, monday).view, 'weekly-planning')
  ...
  assert.equal(openPendingPlanning({ ...weeklyDone, view: 'today' }, monday).view, 'planning', 'Restart after weekly completion opens pending Daily Planning')
  const finished = { ...weeklyDone, 'daily.completedDate': monday, view: 'backlog' }
  assert.deepEqual(openPendingPlanning(project(normalize(finished)), monday), project(normalize(finished)), 'Completed planning does not interrupt same-day reopening')
  assert.equal(completeWeeklyPlanning(finished, monday).view, 'home', 'Repeating Weekly Planning does not repeat completed Daily Planning')
