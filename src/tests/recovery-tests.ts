import { detachInactiveReferences, restoreInactiveReferences, restoreProject } from './view-command-adapters'
...
export async function testRecovery() {
  let requestId = ''
  const coordinator = new FlushCoordinator((id) => {
    requestId = id
  }, 15)
  ...
  assert.equal(
    coordinator.acknowledge(requestId, true),
    false,
    'Late acknowledgements cannot close a newer request',
  )
  const crashed = coordinator.request()
  coordinator.rendererGone()
  assert.equal(await crashed, false)
  const acknowledged = coordinator.request()
  coordinator.acknowledge(requestId, true)
  assert.equal(await acknowledged, true)
  const base = emptyWorkspace('2026-09-07')
  const local = structuredClone(base)
  local.fields.view = 'week'
  const remote = structuredClone(base)
  remote.fields.planningStep = 2
  ...
  fields.tasks = [
    {
      ...
      actualMinutes: 18,
      comments: [{ id: 'c', text: 'History' }],
      recurrence: rule,
      recurrenceSeriesId: 'series',
      recurrenceIndex: 0,
      recurrenceStartDateKey: '2026-09-07',
    },
  ]
  ...
  assert.ok(
    generated.every(
      (e) =>
        !(e.data.content as Data).complete &&
        !(e.data.content as Data).notes &&
        !(e.data.content as Data).actualMinutes,
    ),
  )
  const deleted = structuredClone(extended)
  deleted.entities = deleted.entities.filter((e) => e.id !== generated[0]!.id)
  assert.ok(
    !extendRecurrences(deleted, '2027-09-09').entities.some((e) => e.id === generated[0]!.id),
    'Deleted occurrences never regenerate',
  )
  const moved = structuredClone(remembered)
  moved.entities.find((e) => e.id === 'repeat')!.data.lane = 'date:2030-01-01'
  assert.equal(
    rememberRecurrenceProgress(moved).fields.recurrenceProgress &&
      (rememberRecurrenceProgress(moved).fields.recurrenceProgress as Data).series,
    '2026-09-07',
    'Rescheduling cannot advance logical generation progress',
  )
  const renamed = structuredClone(remembered)
  ;(renamed.entities.find((e) => e.id === 'repeat')!.data.content as Data).title = 'Individual change'
  assert.equal(
    (
      rememberRecurrenceProgress(renamed, remembered).entities.find((e) => e.id === 'repeat')!.data
        .content as Data
    ).recurrenceEdited,
    true,
  )
  const soleDeleted = structuredClone(remembered)
  soleDeleted.entities = soleDeleted.entities.filter((e) => e.id !== 'repeat')
  assert.ok(
    extendRecurrences(soleDeleted, '2027-09-08').entities.some((e) => e.kind === 'task'),
    'Deleting the only materialized occurrence must retain the independent series definition',
  )
  ...
  assert.equal(
    recurrenceDateKeys(
      '2026-01-01',
      { ...rule, end: { type: 'on', date: '2026-01-03' } } as never,
      '2028-01-01',
    ).length,
    3,
  )
  ...
  assert.ok(
    renewed.entities
      .filter((e) => e.kind === 'task' && !exception.entities.some((old) => old.id === e.id))
      .every(
        (e) => (e.data.content as Data).title === 'Recurring work' && (e.data.content as Data).minutes === 30,
      ),
    'Last occurrence exceptions must not contaminate later renewal',
  )
  const organization = project(base)
  organization.tasks = [{ id: 'same-1', title: 'Same title', minutes: 15 }]
  organization.archivedObjectives = [
    {
      id: 'archived',
      channel: 'Work',
      title: 'Archived',
      archivePosition: 0,
      archiveWeeklyOrderIndex: 0,
      archivedSeriesIds: ['series'],
      tasks: [{ id: 'mirror', taskId: 'same-1', title: 'Same title' }],
    },
  ]
  ...
  assert.ok(
    !normalize(detached.fields).entities.some((e) => e.kind === 'task' && e.id === 'same-1'),
    'Inactive references cannot resurrect deleted tasks',
  )
  const undone = restoreInactiveReferences(detached.fields, detached.removed)
  assert.ok(
    normalize(undone).entities.some((e) => e.id === 'same-1'),
    'Undo restores inactive project references',
  )
  ...
  assert.equal(
    (((restoredProject.recurrenceDefinitions as Data).series as Data).task as Data).objectiveId,
    'archived',
  )
  assert.ok((restoredProject.weeklyObjectiveOrder as string[]).includes('archived'))
  const review = syncAccomplishedObjectiveTasks(
    [
      {
        tasks: [
          { taskId: 'a', title: 'Same', minutes: 1 },
          { taskId: 'b', title: 'Same', minutes: 2 },
        ],
      },
    ],
    [
      {
        tasks: [
          { id: 'a', title: 'Renamed', complete: true, minutes: 30 },
          { id: 'b', title: 'Same', complete: true, minutes: 60 },
        ],
      },
    ],
  )
  assert.deepEqual(
    (review[0]!.tasks as Data[]).map((task) => [task.title, task.minutes]),
    [
      ['Renamed', 30],
      ['Same', 60],
    ],
    'Identical titles and renamed tasks keep canonical identity',
  )
  ...
  try {
    ...
    ;(invalid.entities[0]!.data.content as Data).notes = 'x'.repeat(200001)
    assert.throws(() => db.commitWorkspace(changes(start, invalid, 'invalid')), /Text is too long/)
    const corrected = structuredClone(start)
    ;(corrected.entities[0]!.data.content as Data).notes = 'Corrected'
    ...
    const preserved = (await readdir(directory)).find((name) =>
      name.startsWith('pending-edits.json.preserved-'),
    )!
    ...
    const stored = db.loadWorkspace(),
      profiled = structuredClone(stored)
    ...
    const badProfile = structuredClone(db.loadWorkspace())
    badProfile.fields.profile = { displayName: ' ', avatar: 'https://tracking.invalid/avatar' }
    ...
    addFile('canceled')
    db.discardAttachment('canceled')
    assert.equal(db.readAttachment('canceled'), undefined)
    addFile('saved-file')
    const attachmentBase = db.loadWorkspace(),
      withFile = structuredClone(attachmentBase)
    ;(withFile.entities[0]!.data.content as Data).comments = [
      { attachment: { id: 'saved-file', name: 'draft.bin', size: bytes.length } },
    ]
    db.commitWorkspace(changes(attachmentBase, withFile, 'attach-file'))
    db.discardAttachment('saved-file')
    assert.ok(db.readAttachment('saved-file'), 'Discard must not delete a saved attachment')
    addFile('old-orphan')
    addFile('open-draft')
    addFile('recovery-file')
    ...
    assert.ok(
      db.readAttachment('saved-file') &&
        db.readAttachment('open-draft') &&
        db.readAttachment('recovery-file'),
      'Saved, active drafts and pending recovery bytes survive collection',
    )
    const restored = db.loadWorkspace()
    const bad = join(directory, 'bad.sqlite')
