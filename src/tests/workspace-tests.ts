...
import { normalize, project, applyChanges, changes, validateCommit, type Data } from '../domain/workspace'
...
const canonical = (doc: WorkspaceDocument) =>
  JSON.stringify({
    ...doc,
    entities: [...doc.entities].sort((a, b) => (a.kind + a.id).localeCompare(b.kind + b.id)),
  })
export function testWorkspaceDomain() {
  ...
  assert.equal(
    canonical(normalize(project(seed))),
    canonical(seed),
    'Canonical entities must survive projection roundtrip',
  )
  ...
  task.comments = [
    { id: 'comment', text: 'Full comment', attachment: { id: 'test-file', name: 'reference.pdf', size: 10 } },
  ]
  task.activity = [{ id: 'activity', label: 'You edited this', timestamp: 123 }]
  task.recurrence = {
    ...recurrenceForPreset('weekly', '2026-07-14'),
    interval: 2,
    end: { type: 'after', count: 4, date: '' },
  }
  task.actualMinutes = 27
  const subtask = (task.subtasks as Data[])[0]!
  subtask.title = 'Edited subtask'
  subtask.actualMinutes = 12
  ...
  assert.deepEqual(
    (restored.tasks as Data[]).find((t) => t.id === 'main'),
    task,
  )
  for (const key of [
    'daily.planText',
    ...
    assert.deepEqual(restored[key], fields[key])
  assert.equal(
    saved.entities.filter((e) => e.kind === 'task' && e.id === 'main').length,
    1,
    'Mirrors must not duplicate a task',
  )
  ...
  assert.throws(
    () => validateCommit({ ...command, put: [{ kind: 'sql', id: 'x', data: {} }] }),
    /entity kind/,
  )
  assert.throws(
    () =>
      applyChanges(seed, {
        ...
        put: [
          ...command.put,
          {
            kind: 'event',
            id: 'invalid',
            data: { position: 0, content: { id: 'invalid', start: 1440, end: 1500 } },
          },
        ],
      }),
    /calendar event/,
  ...
  try {
    ...
    const write = {
      ...
      put: [],
      remove: [],
      fields: { ...current.fields, view: 'today' },
    }
    const result = database.commitWorkspace(write)
    assert.deepEqual(
      database.commitWorkspace(write),
      result,
      'Lost acknowledgements must be safely retryable',
    )
    assert.equal(database.loadWorkspace().revision, result.revision, 'Retry must not apply twice')
    assert.throws(
      () => database.commitWorkspace({ ...write, fields: { ...write.fields, view: 'home' } }),
      /reused/,
    )
          ...
          put: [
            {
              ...
              data: {
                ...original.data,
                content: { ...(original.data.content as Data), title: 'Must not save' },
              },
            },
            { kind: 'event', id: 'bad', data: { position: 0, content: { id: 'bad', start: -1, end: 3 } } },
          ],
          remove: [],
          fields: write.fields,
        }),
    ...
    assert.equal(
      canonical(database.loadWorkspace()),
      beforeFailure,
      'Rejected batches must roll back every related entity',
    )
  } finally {
    database.close()
  }
