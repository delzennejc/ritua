...
import {
  ...
  previousWeekDays,
  rollWorkspaceDate,
} from '../domain/live-calendar'
import { emptyWorkspace } from '../domain/production-workspace'
import { normalize, project, type Data } from '../domain/workspace'
import { testTaskCompletionPersistence as testTaskCompletion } from './task-completion-persistence-tests'
...
export function testLiveWorkspace() {
  ...
  assert.deepEqual(
    previousWeekDays('2026-09-07').map((day) => day.dateKey),
    ['2026-08-31', '2026-09-01', '2026-09-02', '2026-09-03', '2026-09-04', '2026-09-05', '2026-09-06'],
  )
  assert.ok(calendarDaysAround('2027-01-01').some((day) => day.dateKey === '2026-12-31'))
  assert.equal(
    emptyWorkspace().entities.filter((e) => e.kind !== 'area').length,
    0,
    'Production must start without sample work',
  )
  ...
  fields.datedTasksByDate = {
    '2027-01-01': [{ id: 'new', title: 'New year work', complete: false, minutes: 45 }],
  }
  ...
  assert.equal(
    (((after.ritualHistory as Data)['2026-12-31'] as Data).daily as Data)['daily.planText'],
    'Saved year-end plan',
  )
  ...
  assert.equal(
    (((nextWeek.ritualHistory as Data)['2026-12-28'] as Data).weekly as Data)['weekly.planText'],
    'Saved week plan',
  )
  ...
  try {
    ...
    assert.deepEqual(
      first.entities
        ...
        .map((entity) => (entity.data.content as Data).label)
        .sort(),
      ['Personal', 'Work'],
    )
    assert.ok(
      first.entities.every((entity) => entity.kind === 'area'),
      'Fresh production starts without sample tasks or projects',
    )
    fresh.close()
    ...
    assert.deepEqual(
      ...
      ['app_metadata', 'attachments', 'workspace_entities', 'workspace_receipts', 'workspace_state'],
    )
    rawFresh.close()
