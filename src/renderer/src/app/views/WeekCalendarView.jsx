...
export function WeekCalendarView({
  ...
  const adjacentDateKeys = [-7, 7]
    .map((dayOffset) => dateKeyAfterDays(selectedDateKey, dayOffset));
  const pageDateKeys = [
  ...
  const commitVisibleWeek = (event) => {
    ...
    scrollSettleTimerRef.current = setTimeout(() => {
      ...
      const nextPageIndex = pages.reduce((closestIndex, page, pageIndex) => (
  ...
  useLayoutEffect(() => {
    ...
    scrollElements.forEach((element) => element.addEventListener("scroll", synchronizeScroll, { passive: true }));
        ...
        {pageDateKeys.map((pageDateKey) => (
          <div className="week-calendar-days" data-week-calendar-page={pageDateKey} key={pageDateKey}>
            {weekDateKeysFor(pageDateKey).map((dateKey) => {
              const dayTasks = dateKey === CURRENT_DATE_KEY
                ? tasks
                : datedTasksByDate[dateKey] || [];
