...
const resolveAreaFolder = (areas, channel) => {
  ...
  return (
    areas.find((area) => area.label === channel) ||
    (defaultFolder && areas.find((area) => area.id === defaultFolder.id)) ||
    defaultFolder || { label: channel, color: 'var(--muted)' }
  )
}

export function AreaFoldersProvider({ areas = DEFAULT_AREAS, children }) {
  return <AreaFoldersContext.Provider value={areas}>{children}</AreaFoldersContext.Provider>
}
