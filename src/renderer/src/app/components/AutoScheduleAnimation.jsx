...
function shredCalendarEvent(target, animate, finish) {
  ...
  animate(paper, [{ transform: 'translateY(0)' }, { transform: `translateY(${height}px)` }], {
  ...
  for (let index = 0; index < stripCount; index += 1) {
    ...
    animate(
      ...
      [
        ...
        {
          opacity: 1,
          transform: `translate3d(0, ${height}px, 0) rotate(0deg)`,
          offset: feedDuration / duration,
          easing: 'cubic-bezier(.4, 0, 1, 1)',
        },
        {
          opacity: 0,
          transform: `translate3d(${sway}px, ${height + 48 + (index % 3) * 7}px, 0) rotate(${sway}deg)`,
        },
      ],
  ...
  const closing = animate(
    ...
    {
      duration: 140,
      delay: feedDelay + feedDuration + longestFall - 60,
      easing: 'ease-in',
      fill: 'forwards',
    },
  )
    ...
    const animateCalendar = () => {
      ...
      if (initialRect.top < viewportRect.top + 32 || initialRect.bottom > viewportRect.bottom - exitSpace) {
        viewport.scrollTop +=
          initialRect.top -
          viewportRect.top -
          Math.max(32, (viewport.clientHeight - initialRect.height - exitSpace) / 2)
      }
      ...
      if (ghost) {
        ...
        const deltaY =
          targetRect.top +
          Math.min(targetRect.height, viewport.clientHeight - 32) / 2 -
          (origin.rect.top + origin.rect.height / 2)
        ...
        animate(
          ...
          [
            ...
            {
              transform: `translate3d(${deltaX * 0.4}px, ${deltaY * 0.4 - 34}px, 0) scale(.92) rotate(-3deg)`,
              opacity: 0.95,
              offset: 0.45,
            },
            {
        ...
        if (origin.element.isConnected) {
          animate(origin.element, [{ opacity: 0.18 }, { opacity: 1 }], {
