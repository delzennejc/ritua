...
export function ObjectiveDetails({
  ...
  const taskRows = useMemo(() => {
    const linkedCanonicalEntries = canonicalEntries.filter(({ task }) => task.objectiveId === objective.id)
    ...
    const canonicalRows = linkedCanonicalEntries.map((canonicalEntry) => {
      const objectiveTask = objectiveTasks.find(
        (task) =>
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const objectiveTitle = Array.from(document.querySelectorAll('[data-objective-title-id]')).find(
        (element) => element.dataset.objectiveTitleId === objective.id,
      )
      const fallback = objectiveTitle || document.querySelector('.app-workspace button:not([disabled])')
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const taskTitle = Array.from(content.querySelectorAll('[data-objective-task-open-id]')).find(
        (element) => element.dataset.objectiveTaskOpenId === returnTaskFocusId,
      )
      taskTitle?.focus?.()
    })
    ...
    >
      ...
      >
        <h2 className="sr-only" id={titleId}>
          Project details for {objective.title}
        </h2>
          ...
          <div className="task-details-actions objective-details-actions">
            <Dropdown
              ...
              items={[
                {
                  id: 'complete',
                  label: objective.complete ? 'Mark incomplete' : 'Mark complete',
                  icon: objective.complete ? <Circle size={16} /> : <CheckCircle size={16} />,
                  onSelect: onToggle,
                },
                ...(objective.focusedThisWeek !== false && onRemoveFromWeek
                  ? [
                      {
                        id: 'remove-week',
                        label: 'Remove from this week',
                        icon: <CalendarX size={16} />,
                        onSelect: onRemoveFromWeek,
                      },
                    ]
                  : []),
                ...(onArchive
        ...
        <div ref={contentRef} className="objective-details-content">
          <section className="objective-details-primary">
            <div className="objective-details-kicker">
              ...
              {taskRows.length ? (
                <small>
                  {completeTaskCount} of {taskRows.length} tasks complete
                </small>
              ) : null}
            </div>
                ...
                return (
                  <div className={allTasksComplete ? 'complete' : ''} key={day.dateKey}>
                    ...
                    <div
                      ...
                      aria-label={totalTasks ? `${day.label} task completion` : undefined}
                      aria-valuemin={totalTasks ? 0 : undefined}
                      aria-valuemax={totalTasks || undefined}
                      aria-valuenow={totalTasks ? completedTasks : undefined}
                      aria-valuetext={
                        totalTasks ? `${completedTasks} of ${totalTasks} tasks complete` : undefined
                      }
                    >
            ...
            {taskDraftOpen ? (
              <form
                ...
                onBlur={(event) => {
                  if (!taskDraftTitle.trim() && !event.currentTarget.contains(event.relatedTarget)) {
                    cancelTaskDraft()
                  }
                }}
                onKeyDown={(event) => {
            ...
            >
              ...
              <div>
                ...
                />
                {attachmentName ? (
                  <span className="task-details-attachment-name">
