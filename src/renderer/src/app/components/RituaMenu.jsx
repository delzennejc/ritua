...
function ProjectTaskProgress({ tasks = [] }) {
  ...
  return (
    <span
      aria-hidden="true"
      className={`project-task-progress ${isComplete ? "complete" : ""}`}
    >
      {isComplete ? (
...
export function RituaMenu({
  ...
  const areaDraftColor = AREA_COLOR_OPTIONS.find((option) => (
    option.id === areaDraftColorId
  ...
  const submitAreaDraft = (event) => {
    ...
    requestAnimationFrame(() => {
      document.querySelector(
        `[data-area-navigation-id="${createdArea.id}"]`,
      )?.focus();
    });
  };
    ...
    <>
      <aside className="sidebar ritua-menu">
        ...
        <nav
          ...
          onScroll={() => setProjectTooltip(null)}
        >
        {main.map((item) => <NavItem key={item.id} item={item} />)}
        <p className="nav-label">Rituals</p>
        {rituals.map((item) => <NavItem key={item.id} item={item} />)}
                        ...
                        {projects.map((project) => {
                          ...
                          return (
                            <button
                              ...
                              onPointerEnter={(event) => showProjectTooltip(project.title, event.currentTarget)}
            ...
            >
              <div className="area-draft-main">
                <Folder
                  size={15}
                  weight="fill"
                  style={{ color: areaDraftColor.color }}
                />
                <input
