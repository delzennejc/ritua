...
export function Dropdown({
  ...
  useLayoutEffect(() => {
    ...
    const position = () => {
      ...
      const top =
        bounds.bottom + 7 + height <= window.innerHeight - 8 ? bounds.bottom + 7 : bounds.top - height - 7
    ...
    ;(focused ?? (children ? menu.querySelector('button, input, select, textarea') : null) ?? menu).focus({
      preventScroll: true,
    })
    ...
    const dismiss = (event) => {
      if (
        ...
        event.target.closest?.('[data-dropdown-root]')?.dataset.dropdownRoot !== rootId
      )
                ...
                {children ??
                  items.map((item) => (
                    <button
                      ...
                      key={item.id}
                      type="button"
                      className={`dropdown-option${item.danger ? ' dropdown-option-danger' : ''}`}
                      role={item.role ?? 'menuitem'}
                      disabled={item.disabled}
                      aria-checked={
                        item.role === 'menuitemcheckbox' || item.role === 'menuitemradio'
                    ...
                    >
                      ...
                      {item.checked ? (
                        <Check className="dropdown-check" size={14} weight="bold" aria-hidden="true" />
                      ) : null}
                    </button>
                  ))}
...
export function ChoiceDropdown({
  ...
  className = '',
  trigger,
  triggerClassName = 'choice-dropdown-trigger',
  menuWidth = 280,
}) {
