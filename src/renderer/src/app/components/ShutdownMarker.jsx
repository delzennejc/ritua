...
export function ShutdownMarker({ event, setEvents, timelineScrollRef, positionForMinutes }) {
  ...
  const saveMinute = (start) =>
    setEvents?.((items) =>
      items.map((item) =>
        item.id === event.id && item.dateKey === event.dateKey ? { ...item, start, end: start } : item,
      ),
    )
  ...
  const updatePreview = () => {
    ...
    drag.minute = clampMinute(
      snapCalendarMinutes(drag.start + ((drag.y - drag.initialY + scrollDelta) / CALENDAR_HOUR_HEIGHT) * 60),
    )
