...
const isTaskCreatedActivity = (activity) => (
  activity?.kind === "task-created"
);

export const isNoteEditedActivity = (activity) => (
  activity?.kind === "note-edited"
);

export const taskActivityWithCreation = (task) => {
  const activity = task.activity || [];
  return activity.some(isTaskCreatedActivity)
    ? activity
    : [taskCreatedActivity(task), ...activity];
};

export const appendTaskActivity = (task, activity) => ([
  ...taskActivityWithCreation(task),
  activity,
]);
