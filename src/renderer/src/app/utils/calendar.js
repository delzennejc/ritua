...
export const clampCalendarEnd = (end, start) => (
  Math.max(
    start + CALENDAR_MIN_EVENT_MINUTES,
    Math.min(CALENDAR_DAY_MINUTES, end),
  )
);
...
export const calendarEndAfterResize = ({ start, end, deltaY, scrollDelta = 0 }) => (
  clampCalendarEnd(
    snapCalendarMinutes(
      end + ((deltaY + scrollDelta) / CALENDAR_HOUR_HEIGHT) * 60,
    ),
    start,
  )
);
...
export const calendarStartAfterMove = ({ start, duration, deltaY, scrollDelta = 0 }) => (
  clampCalendarStart(
    snapCalendarMinutes(
      start + ((deltaY + scrollDelta) / CALENDAR_HOUR_HEIGHT) * 60,
    ),
    duration,
  )
);
...
export const calendarStartAtPointer = ({ pointerY, timelineTop, duration }) => (
  clampCalendarStart(
    snapCalendarMinutes(
      ((pointerY - timelineTop) / CALENDAR_HOUR_HEIGHT) * 60,
    ),
    duration,
  )
);

export const firstAvailableCalendarStart = (
  events,
  duration,
  preferredStart = 8 * 60,
) => {
  ...
  const sortedEvents = events
    .filter((event) => event.kind !== "shutdown" && Number.isFinite(event.start) && Number.isFinite(event.end))
    .map((event) => ({
  ...
  return candidate + resolvedDuration <= CALENDAR_DAY_MINUTES
    ? candidate
    : null;
};

export const nextAvailableCalendarStart = (
  events,
  duration,
  dateKey,
  { taskId, now = new Date() } = {},
) => {
  const preferredStart = dateKey === CURRENT_DATE_KEY
    ? Math.ceil(currentDayMinute(now) / CALENDAR_SNAP_MINUTES) * CALENDAR_SNAP_MINUTES
    : 8 * 60;
  const dayEvents = events.filter((event) => (
    (!taskId || event.id !== taskId)
    && (event.dateKey || CURRENT_DATE_KEY) === dateKey
  ));
...
export function groupTaskCompletions(tasks, clusterMinutes = COMPLETION_CLUSTER_MINUTES) {
  ...
  completions.forEach((completion) => {
    ...
    if (
      currentGroup
      && completion.minute - currentGroup.startedAtMinute <= clusterMinutes
    ) {
