...
export function useAttachmentDraft() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (current.current) discard(current.current)
    }
  }, [])
...
export function AttachmentPicker({ onChange }: { onChange: (file: AttachmentInfo) => void }) {
  ...
  const choose = async () => {
    ...
    setBusy(true)
    try {
      const file = await window.ritua!.chooseAttachment()
  ...
  return (
    <>
      <label className="task-details-attachment">
        <Paperclip size={19} />
          ...
          <input
            ...
            onChange={(event) => {
              ...
              if (file.size > 25 * 1024 * 1024) {
                setError('Choose a file no larger than 25 MB.')
                return
              }
              const id = crypto.randomUUID()
              previewFiles.set(id, file)
  ...
  const save = async () => {
    ...
    try {
      if (window.ritua) await window.ritua.exportAttachment(attachment.id)
        ...
        if (!file) throw new Error('This preview file is no longer available.')
