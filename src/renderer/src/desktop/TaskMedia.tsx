  ...
  return createPortal(
    ...
    >
      <header className="task-image-viewer-toolbar">
        ...
        <button type="button" onClick={onClose} aria-label="Close image viewer" autoFocus>
          <X size={22} />
        </button>
      </header>
      <div
        className="task-image-viewer-stage"
        onClick={(event) => {
      ...
      {error ? (
        <p className="task-image-viewer-error" role="alert">
          {error}
        </p>
      ) : null}
    </dialog>,
...
function MediaTile({ attachment, onRemove }: { attachment: AttachmentInfo; onRemove: () => void }) {
  ...
  const copyImage = async () => {
    ...
    try {
        ...
        const png = (async () => {
          ...
          try {
            ...
            return await new Promise<Blob>((resolve, reject) =>
              canvas.toBlob(
                (blob) => (blob ? resolve(blob) : reject(new Error('This image could not be copied.'))),
                'image/png',
              ),
            )
          } finally {
      ...
      setCopyState('idle')
      setCopyError(cause instanceof Error ? cause.message : 'This image could not be copied.')
    }
  }
  useEffect(() => {
    let active = true
    const file = previewFiles.get(attachment.id)
    const promise = window.ritua
      ? window.ritua.readTaskImage(attachment.id)
      : file
        ? readDataUrl(file)
  ...
  return (
    <li className="task-media-tile">
      ...
      >
        {url && !error ? (
          <img src={url} alt={attachment.name} onError={() => setError('Image unavailable')} />
        ) : (
      ...
      <div className="task-media-caption">
        ...
        <span className="task-media-actions">
          <button
            type="button"
            aria-label={`Copy image ${attachment.name}`}
            title={copyState === 'copied' ? 'Copied!' : 'Copy image'}
            disabled={copyState === 'copying' || !url || Boolean(error)}
...
export function TaskMedia({
  ...
  const latest = useRef({ media, onChange })
  latest.current = { media, onChange }
  ...
  const importFiles = async (files: File[]) => {
    if (importing.current) {
      setError('Wait for the current images to finish importing.')
      return
    }
    ...
    try {
      for (const file of files) {
        ...
        if (!mounted.current) {
          await window.ritua?.discardAttachment(attachment.id)
          break
        }
  ...
  useEffect(() => {
    ...
    dialog.addEventListener('dragenter', enter)
    dialog.addEventListener('dragover', over)
    dialog.addEventListener('dragleave', leave)
    dialog.addEventListener('drop', drop)
    return () => {
      mounted.current = false
      dialog.removeEventListener('paste', paste)
      dialog.removeEventListener('dragenter', enter)
      dialog.removeEventListener('dragover', over)
      dialog.removeEventListener('dragleave', leave)
      dialog.removeEventListener('drop', drop)
    }
  }, [dialogRef])
  return (
    <section
      className={`task-media ${dragging ? 'dragging' : ''}`}
      aria-label="Media library"
      aria-busy={busy}
    >
      <header>
        <span>
          <Images size={17} /> Media {media.length ? <small>{media.length}</small> : null}
        </span>
            ...
            <MediaTile
              ...
              onRemove={() =>
                latest.current.onChange(
                  latest.current.media.filter((value) => value.attachment.id !== item.attachment.id),
                )
              }
            />
          ))}
      ...
      <p className="task-media-hint" role="status">
        {dragging
          ? 'Drop images to add them to this task'
          : busy
            ? 'Adding images…'
            : 'Paste or drop images anywhere in Task details'}
      </p>
      {error ? (
        <p className="task-media-error" role="alert">
          {error}
