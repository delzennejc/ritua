...
async function launch(flag = '--smoke-test', dataDirectory = directory) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(electron, process.env.RITUA_TEST_EXECUTABLE ? [flag] : ['.', flag], { env, stdio: ['ignore', 'pipe', 'pipe'] })
    let output = ''
    const timer = setTimeout(() => { child.kill(); reject(new Error('Electron smoke test timed out')) }, 60000)
